...
(async () => {
  ...
  client.on('connect', async () => {
    ...
    // ein Array in redis schreiben funktioniert so nicht set überschreibt den ganzen schlüssel. lpush rpush?
    await myArray.forEach((item) => {
      client.set(listKey, item);
    });

    const result = await client.get(listKey, 0, -1);

    console.log('Array aus Redis abgerufen:', result);



    await client.hSet('keytest', '0', 'value');
    await client.hSet('keytest', '1', 'value2');
    await client.hSet('keytest', '2', 'value3');


// Ein leeres Objekt erstellen
const testObjekt = {};

// Schleife, um die Array-Elemente in das Objekt einzufügen
/*
for (let i = 0; i < myArray.length; i++) {
  const key = `element${i + 1}`;
  const value = myArray[i];
  testObjekt[key] = value;
}
*/
for (let i = 0; i < myArray.length; i++) {
  
  await client.hSet('keytest2', i, myArray[i]);
}



    let test2 = await client.hGetAll('keytest2');
    console.log(test2);

    let test3 = await client.hVals('keytest2');
    console.log(test3);


    
    // Setzen Sie den Wert in Redis
    await client.set('key', 'Testwert');
    console.log("te");
     ...
     const meinWert = await client.get('key');
